...
### 修复Bug
- 修复了图片缩放比例计算问题
  - 修改了 `scale` 的计算方式，使其相对于用户导入时设置的尺寸而非原始尺寸
  - 添加了 `initial_width` 和 `initial_height` 属性记录用户设置的初始尺寸
  - 更新了 `to_dict` 方法的 `scale` 计算逻辑
  - 确保导入图片设置尺寸后，如果不做额外调整，导出的 scale 值为 1.0

### 新增功能
- 修改了图片尺寸保存逻辑
  - 将JSON数据中保存"原始尺寸+缩放值"同时也直接保存"实际显示尺寸"
  - 优化 `ImageItem` 类的 `to_dict` 方法，计算并保存实际显示尺寸和缩放信息
  - 保留 `scale` 字段，确保数据的完整性和兼容性
  - 修改 `open_file` 方法，使用新的尺寸格式加载图片
  - 保留原始尺寸信息，便于后续处理
