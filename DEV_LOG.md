...
### 代码修改
- 修改了 `ui/tool_panel.py` 中的 `add_image_signal` 和 `on_add_image_clicked` 方法
  - 更新了信号参数，添加了宽度和高度参数
  - 修改了图片添加对话框的标题和提示文本
  - 添加了获取图片大小设置的方法

- 修改了 `ui/main_window.py` 中的 `add_image` 方法
  - 添加了宽度和高度参数
  - 实现了图片大小的设置功能
  - 添加了异常处理
  - 添加了 `on_add_image` 方法处理添加图片信号
...
### 修复了 `add_image` 方法中的错误，将 `set_size` 替换为 `resize` 方法，以支持图片大小设置。

### 新增功能
- 修复了 `MainWindow` 类中缺少 `update_material_list` 方法的问题
  - 添加了 `update_material_list` 方法，用于更新材质球列表
  - 在添加贴图后自动更新材质球列表
  - 更新状态栏显示当前贴图数量

### 新增功能
- 修复了导出功能的信号参数问题
  - 修改了 `ToolPanel` 类中的 `export_signal` 信号定义，添加了 `export_path` 参数
  - 确保导出功能正确传递 `preset_id` 和 `export_path` 参数
  - 优化了导出功能的错误处理
- 修复了 `MainWindow` 类中缺少 `update_material_list` 方法的问题
  - 添加了 `update_material_list` 方法，用于更新材质球列表
  - 在添加贴图后自动更新材质球列表
